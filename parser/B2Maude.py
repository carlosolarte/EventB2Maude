...
class BMaude(EventBListener):
    ...
    def exitInitst(self, ctx:EventBParser.InitstContext):
        ...
        # Initialization are simple assignments (for the moment)
    ...
    def contextWrapper (self):
        ...
        '''
        return textwrap.dedent(f''' < $$CNAME : Context | sets: ($$Sets),
                                                          constants: ({self.constantsCtx()}) >''')
    ...
    def ruleWrapper (self):
        ...
        '''
        return textwrap.dedent(f''' {self.contextWrapper()}
